...
def connected_msg():
    ...
    connections_count = len(manager.rooms['/']) - 1 # 统计当前连接数
...
def send_msg(nlu_result, func, frame, seq, cost, status):
    ...
    nlu_result["frame"] = frame ## 内容帧
    nlu_result["seq"] = seq# 序列号
    nlu_result["cost"] = cost # 耗时
    nlu_result["status"] = status # 状态: 0开始,1中间,2结束,-1错误
...
def inference(req):
    ...
    sender_id = json_info.get("sender_id", "test") #用户id
    trace_id = json_info.get("trace_id", "123") #一句话的id
#nlu的应答模板
    nlu_template = {
        "query": query,
        "tarce_id": trace_id,
